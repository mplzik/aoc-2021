...
fn main() {
    ...
    let mut xprobe = [
        1i32 * xrange.start().signum(),
        if xrange.start().abs() > xrange.end().abs() {
            *xrange.start()
        } else {
            *xrange.end()
        },
    ...
    for ivx in xprobe[0]..=xprobe[1] {
        // -300 is a guesstimate, needs a proper bound.
        let mut yprobe = [
            1i32 * yrange.start().signum(),
            -300 * yrange.start().signum(),
        ];
        yprobe.sort();
