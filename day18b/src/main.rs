...
fn main() {
    ...
    for first in 0..numbers.len() {
        for second in 0..numbers.len() {
            ...
            let (remaining, number1) = parse_num(&mut trees, usize::MAX, &numbers[first]);
            assert_eq!(remaining, &[]);
            let (remaining, number2) = parse_num(&mut trees, usize::MAX, &numbers[second]);
            assert_eq!(remaining, &[]);

            let sum = sum_sfn(&mut trees, as_ref(&number1), as_ref(&number2));
            let mag = magnitude(&trees, as_ref(&sum));
            if mag > max_mag {
                max_mag = mag;
            }
        }
