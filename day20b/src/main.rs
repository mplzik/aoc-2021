...
fn enhance(map: &Vec<Vec<char>>, p: &Vec<char>, outside: char) -> (Vec<Vec<char>>, char) {
    ...
    for y in 0isize..(map.len()+4) as isize {
        ...
        for x in 0isize..(map[0].len()+4) as isize {
            ...
            let mut idx = 0;
            for neighbor in [[-1, -1], [-1, 0], [-1, 1], [0, -1], [0, 0], [0, 1], [1,-1], [1, 0], [1, 1]] {
                idx *= 2;
    ...
    (ret, match outside { '.' => p[0b000000000], '#' => p[0b111111111], _ => panic!("Unknown outside char!") })
}
...
fn main() {
    ...
    let program = stdin.lock().lines().next().unwrap().unwrap().chars().collect::<Vec<char>>();
